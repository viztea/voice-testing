...
import { ByteWriter, floorDiv, randomBytes, range, uint16, uint32, uint32toBytes, uint8, withIndex } from "../tools/mod.ts";
...
export async function generateMasterKey(): Promise<MasterKey> {
    ...
    return {
        salt,
        secret,
        secretBytes: new Uint8Array(await crypto.subtle.exportKey("raw", secret)),
    };
}
...
function generateCounter(cursor: ByteWriter, seq: uint16, roc: uint32, ssrc: uint32, salt: Uint8Array) {
...
export function encryptCtr(key: CryptoKey, payload: Uint8Array, counter: Uint8Array): Promise<ArrayBuffer> {
    return crypto.subtle.encrypt({ name: "AES-CTR", counter, length: 64 }, key, payload);
}
